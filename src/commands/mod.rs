...
/// CSV document builders for `health --csv`, `pool --csv`, `status --csv` and
/// `ps --all --csv` (FR-007 / AC-007.82 / AC-007.83).
///
/// Re-exported at the `commands` root so the existing
/// `sharecli::commands::render_*_csv_body` paths keep resolving.
pub use render_csv::{
    render_health_csv_body, render_pool_csv_body, render_ps_all_csv_body, render_status_csv_body,
};
...
async fn render_ps_once(
    ...
    if csv {
        ...
        let csv_out = render_csv::append_operator_csv_companions(body, &payload.gate).await?;
        print!("{csv_out}");
        return Ok(());
    }
...
async fn render_status_once(verbose: bool, json: bool, csv: bool, ndjson: bool) -> Result<()> {
    ...
    if csv {
        ...
        let body =
            render_csv::render_status_csv_body(&summary, &harness_rows, &pool_status, used, total);
        let (gate, _) = capture_live_gate_host_watch()?;
        let csv_out = render_csv::append_operator_csv_companions(body, &gate).await?;
        print!("{csv_out}");
        return Ok(());
    }
...
async fn render_pool_once(json: bool, csv: bool, ndjson: bool) -> Result<()> {
    ...
    if csv {
        ...
        let body = render_csv::render_pool_csv_body(&payload);
        let csv_out = render_csv::append_operator_csv_companions(body, &payload.gate).await?;
        print!("{csv_out}");
        return Ok(());
    }
...
async fn render_health_once(
    ...
    if csv {
        let payload = build_health_json().await?;
        let body = render_csv::render_health_csv_body(&payload);
        let csv_out = render_csv::append_operator_csv_companions(body, &payload.gate).await?;
        print!("{csv_out}");
        return Ok(());
    }
